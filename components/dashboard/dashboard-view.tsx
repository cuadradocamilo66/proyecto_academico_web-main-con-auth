...
import { SessionsManagement } from "@/components/sessions/sessions-management"
...
export function DashboardView() {
  ...
  const recentActivity = [
    ...
    .sort((a, b) => new Date(b.createdAt + "T00:00:00").getTime() - new Date(a.createdAt + "T00:00:00").getTime())
    .slice(0, 5)
      ...
      <Tabs defaultValue="overview" className="w-full">
        <TabsList className="bg-muted/50 mb-6 p-1">
          ...
          <TabsTrigger value="sessions" className="gap-2">
            <Play className="h-4 w-4" /> Sesiones en Vivo
          </TabsTrigger>
        </TabsList>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-blue-600 dark:text-blue-500 mt-1">{activeStudents} activos</p>
                  </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-purple-600 dark:text-purple-500 mt-1">{courses.length} en total</p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-green-600 dark:text-green-500 mt-1 flex items-center gap-1">
                      <TrendingUp className="h-3 w-3" /> Rendimiento positivo
                    </p>
                  </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-orange-600 dark:text-orange-500 mt-1">Requieren atención</p>
                  </div>
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            ...
            <Card className="border-none shadow-sm">
              ...
              <CardContent className="space-y-2">
                <Button
                  variant="outline"
                  className="w-full justify-start gap-2 hover:bg-primary hover:text-primary-foreground transition-colors group"
                  onClick={() => {
                    const btn = document.querySelector('[value="sessions"]') as any;
                    if (btn) btn.click();
                  }}
                >
                ...
                <Link href="/calificaciones">
                  <Button variant="outline" className="w-full justify-start gap-2 hover:bg-primary hover:text-primary-foreground transition-colors">
                    <FileText className="h-4 w-4" /> Agregar Calificaciones
                  </Button>
                </Link>
                <Link href="/planeacion">
                  <Button variant="outline" className="w-full justify-start gap-2 hover:bg-primary hover:text-primary-foreground transition-colors">
                    <Calendar className="h-4 w-4" /> Nueva Planeación
                  </Button>
                </Link>
                <Link href="/observaciones">
                  <Button variant="outline" className="w-full justify-start gap-2 hover:bg-primary hover:text-primary-foreground transition-colors">
                    <AlertTriangle className="h-4 w-4" /> Registrar Observación
                  </Button>
                </Link>
                <Link href="/reportes">
                  <Button variant="outline" className="w-full justify-start gap-2 hover:bg-primary hover:text-primary-foreground transition-colors">
                ...
                {coursePerformance.length === 0 ? (
                  ...
                  <ResponsiveContainer width="100%" height={280}>
                    <BarChart data={coursePerformance}>
                      <XAxis dataKey="name" tickLine={false} axisLine={false} fontSize={11} angle={-45} textAnchor="end" height={80} />
                        ...
                        if (active && payload && payload.length) {
                          return (
                            <div className="rounded-lg border bg-card p-3 shadow-lg">
                              ...
                              <p className="text-xs text-muted-foreground">Promedio: <span className="font-semibold">{payload[0].value}</span></p>
                            </div>
                          )
                        }
                        return null
                ...
                {studentsAtRisk.length === 0 ? (
                  ...
                  <div className="space-y-3 max-h-[280px] overflow-y-auto pr-2">
                    {studentsAtRisk.slice(0, 6).map((student) => (
                      <div key={student.id} className="flex items-center gap-3 p-3 rounded-lg border border-orange-200 bg-orange-50">
                        ...
                        <Badge variant="destructive" className="shrink-0">{student.average.toFixed(1)}</Badge>
                      </div>
                    ))}
                  </div>
                )}
